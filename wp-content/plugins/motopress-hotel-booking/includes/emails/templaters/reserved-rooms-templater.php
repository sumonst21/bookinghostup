...
class ReservedRoomsTemplater extends AbstractTemplater {
	...
	public function replaceTag( $match ){
		...
		switch ( $tag ) {
			...
			case 'room_key':
				...
				break;
			/** HostUP - Self-Check-in - 20190322**/
			case 'self_check_in_link':
				if ( isset( $this->reservedRoom ) ) {
					$replaceText = $this->reservedRoom->getMlscSelfCheckInLink();
				}
				break;
			case 'self_check_in_pwd':
				if ( isset( $this->reservedRoom ) ) {
					$replaceText = $this->reservedRoom->getMlscSelfCheckInPwd();
				}
				break;
			/** HostUP end **/
		}
	...
	public function setupTags(){
		$tags = array(
			...
			/** HostUP - Self-Check-in - 20190322**/
			array(
				'name'			 => 'self_check_in_pwd',
				'description'	 => __( 'Self-Check-In Password', 'motopress-hotel-booking' ),
			),
			array(
				'name'			 => 'self_check_in_link',
				'description'	 => __( 'Self-Check-In Link', 'motopress-hotel-booking' ),
			)
			/** HostUP end **/
		);

...
class Coupon {
	 ...
	 * @var \DateTime
	 */
	private $checkInDateAfter;

	/**
	 *
	 * @var \DateTime
	 */
	private $checkOutDateBefore;
	 ...
	 * @return float
	 */
	function calcDiscountAmount( $booking ){
		$discount = 0.0;
		...
		foreach ( $booking->getReservedRooms() as $reservedRoom ) {
			...
			$roomPrice = $reservedRoom->calcRoomPrice( $booking->getCheckInDate(), $booking->getCheckOutDate() );
			$discount += min( $roomPrice * $this->amount / 100, $roomPrice );
		}
	 ...
	 * @return booln
	 */
	function isValidForBookingContents( $booking ){
		...
		if (
			!is_null( $this->checkOutDateBefore ) &&

...
//			),
	...
	private function detectCustomFieldsDefaults(){
		...
		if ( empty( $_GET['mphb_defaults'] ) ) {
			return $defaults;
		}

		$defaults = $_GET['mphb_defaults'];

		if ( !empty( $defaults['_mphb_booking_id'] ) ) {

			$bookingDefaults = $this->retrieveDefaultsFromBooking( (int) $defaults['_mphb_booking_id'] );
	...
	public function getFieldGroups(){
		...
		$paymentFields = array(
			...
			Fields\FieldFactory::create(
				...
				'default'	 => !empty( $defaults['_mphb_amount'] ) ? floatval( $defaults['_mphb_amount'] ) : 0,
				'step'		 => 0.01,
				'min'		 => 0,
				'size'		 => 'price',
				'required'	 => true

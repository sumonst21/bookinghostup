...
class ReservedRoom {
	...
	public function getPriceBreakdown( $checkInDate, $checkOutDate, $coupon = null, $language = null ){
		...
		$total = $roomBreakdown['discount_total'] + $servicesBreakdown['total'] + $feesBreakdown['total'] +
	...
	/** HostUP - Self-Check-in Email Link - 20190322**/

	public function getMlscSelfCheckInLink() {

		$mlsc_scheck_id = $this->getMlscSelfCheckInId();

		foreach ( $mlsc_scheck_id as $post ) {
			$mlsc_scheck_in_link = get_permalink($post->ID);
		}

		return $mlsc_scheck_in_link;

	}

	public function getMlscSelfCheckInPwd() {

		$mlsc_scheck_id = $this->getMlscSelfCheckInId();

			foreach ( $mlsc_scheck_id as $post ) {
				$mlsc_scheck_in_pwd = $post->post_password;
			}
		return $mlsc_scheck_in_pwd;
	}

	public function getMlscSelfCheckInId() {

		$args = array(
			'meta_key'         => 'accommodation',
			'meta_value'       => $this->getRoomId(),
		...
		$mlsc_scheck_post = get_posts( $args );
		return $mlsc_scheck_post;
